...
static INT32 consys_clock_buffer_ctrl(MTK_WCN_BOOL enable)
{
	if (!KERNEL_is_clk_buf_from_pmic())
...
static INT32 consys_hw_vcn18_ctrl(MTK_WCN_BOOL enable)
{
	...
	if (enable) {
		...
		KERNEL_pmic_set_register_value(PMIC_RG_LDO_VCN18_HW0_OP_CFG, 0);
#else
...
static VOID consys_vcn28_hw_mode_ctrl(UINT32 enable)
{
	...
	if (enable) {
#if defined(CONFIG_MTK_PMIC_CHIP_MT6355)
		KERNEL_pmic_set_register_value(PMIC_RG_LDO_VCN28_EN, 1);
		KERNEL_pmic_set_register_value(PMIC_RG_LDO_VCN28_SW_OP_EN, 1);
#else
		KERNEL_pmic_set_register_value(MT6351_PMIC_RG_VCN28_ON_CTRL, 1);
#endif
	} else {
#if defined(CONFIG_MTK_PMIC_CHIP_MT6355)
		KERNEL_pmic_set_register_value(PMIC_RG_LDO_VCN28_EN, 0);
		KERNEL_pmic_set_register_value(PMIC_RG_LDO_VCN28_SW_OP_EN, 0);
#else
