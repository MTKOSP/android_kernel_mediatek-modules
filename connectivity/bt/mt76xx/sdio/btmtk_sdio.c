	...
	do {
		...
		if ((RX_DONE&u32ReadCRValue) && rx_length) {
			...
			if (sdio_header_length != rx_length) {
				pr_err("%s sdio header length %d, rx_length %d mismatch ret =%d\n",
...
static bool btmtk_is_bin_file_mode(uint8_t *buf, size_t buf_size)
{
	...
	/* find string */
	ptr = strstr(p_buf, E2P_MODE);
	if (!ptr) {
		pr_warn("%s: Can't find %s\n", __func__, E2P_MODE);
		ret = false;
		goto out;
	}
	...
	if (*ptr != BIN_FILE_MODE) {
		pr_warn("%s: It's not EEPROM - Bin file mode\n", __func__);
		ret = false;
		goto out;
	}
 ...
 */
	...
	if (push_in_data_len % 4)
		push_in_data_len = ((push_in_data_len / 4) + 1)*4;
					...
					struct btmtk_sdio_card *card)
{
	...
	if (btmtk_sdio_need_load_rom_patch() == PATCH_READY)
		pr_info("%s patchdownload is done\n", __func__);
